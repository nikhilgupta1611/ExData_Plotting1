...
length(mydata1$Global_active_power)
plot(1:2880,mydata1$Global_active_power,type = "l",lty="solid" , ylab = "Global Acive Power",xlab ="FREQUENCY", xaxt = "n")
axis(2, at=seq(0,6,by=2), labels=seq(0,6,by=4) )
axis(1, at=c(0, 2880/2, 2880) , labels = c("Thu" , "Fri", "Sat"))
#plot3
mydata1$Voltage1<-as.numeric(mydata1$Voltage)
length(mydata1$Voltage1)
plot(1:2880,mydata1$Voltage1,type = "l",lty="solid" , ylab = "Voltage",xlab ="datetime", xaxt = "n",yaxt="n")
axis(1, at=c(0, 2880/2, 2880) , labels = c("Thu" , "Fri", "Sat"))
...
axis(1, at=c(0, 2880/2, 2880) , labels = c("Thu" , "Fri", "Sat"))
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),bty = "n",lty="solid",col=c("black","red","blue"),bg="white",lwd=0.5, cex=.3,y.intersp = .4 , yjust = 1 , xjust = 1 , text.width = 800,box.lty = NULL) 
#plot4
mydata1$Global_reactive_power1<-as.numeric(mydata1$Global_reactive_power)
length(mydata1$Global_reactive_power1)
plot(1:2880,mydata1$Global_reactive_power1,type = "l",lty="solid" , ylab = "Global_reactive_power",xlab ="datetime", xaxt = "n",yaxt="n")
axis(1, at=c(0, 2880/2, 2880) , labels = c("Thu" , "Fri", "Sat"))
axis(2, at=seq(0.0,0.5, by=0.1), labels=seq(0.0,0.5, by=0.1) )
range(mydata1$Global_reactive_power1)
